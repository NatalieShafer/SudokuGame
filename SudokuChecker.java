...
public class SudokuChecker{
    ...
    public SudokuChecker(int dimension) {
    ...
    public Sudoku solve(Sudoku input) {
        ...
        for (int y = 0; y < 9; ++y) {
          for (int x = 0; x < 9; ++x) {
            ...
            if(SudokuGrid.solvingArray[count] != -1 && SudokuGrid.solvingArray[count] != solution.get(x, y))
            {                    
              throw new IllegalArgumentException("The cell (x = " + (x+1) + ", y = " + (y+1) + ") with " +"value " + SudokuGrid.solvingArray[count] +  " is incorrect, pleace check the logic.");
            }
            count++;
          }
        }
